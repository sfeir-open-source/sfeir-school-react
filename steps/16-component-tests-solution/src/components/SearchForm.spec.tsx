...
describe('SearchForm', () => {
  ...
  it('text typed in the search input should be emitted', async () => {
    // ARRANGE
    render(<SearchFormWrapper />);

    // ACT
    userEvent.type(screen.getByLabelText('Search'), 'Something');

    // ASSERT
    await expect(screen.findByText('Something')).resolves.toBeVisible();
  });
});
